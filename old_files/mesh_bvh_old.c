									...
									t_triangle_centroid *centroids)
{
	...
	while (i < mesh->triangle_count)
	{
		...
		centrioids[i].centroid.x = (tri->v0.x + tri->v1.x + tri->v2.x) / 3.0;
		centrioids[i].centroid.y = (tri->v0.y + tri->v1.y + tri->v2.y) / 3.0;
		centrioids[i].centroid.z = (tri->v0.z + tri->v1.z + tri->v2.z) / 3.0;
		centrioids[i].original_index = i;
		i++;
	}
}
