...
int any_intersection(t_scene *scene, t_ray ray, double max_dist)
{
	...
	while (current)
	{
		if (current->type == SPHERE)
		{
			...
			// FIXED: Added t_temp > 0.0001 check to prevent self-intersection
			if (ray_sphere_intersect(ray, *sphere, &t_temp) && t_temp > 0.0001 && t_temp < max_dist)
				return (1);
		}
		else if (current->type == CYLINDER)
		{
			t_cylinder *cylinder = (t_cylinder *)(current->data);
			// FIXED: Added t_temp > 0.0001 check
			if (ray_cylinder_intersect(ray, *cylinder, &t_temp) && t_temp > 0.0001 && t_temp < max_dist)
				return (1);
		}
		else if (current->type == PLANE)
		{
			...
			// FIXED: Added t_temp > 0.0001 check
			if (ray_plane_intersect(ray, *plane, &t_temp) && t_temp > 0.0001 && t_temp < max_dist)
				return (1);
		}
		else if (current->type == TRIANGLE)
		{
			...
			// FIXED: Added t_temp > 0.0001 check
			if (ray_triangle_intersect(ray, *triangle, &t_temp) && t_temp > 0.0001 && t_temp < max_dist)
				return (1);
		}
		else if (current->type == MESH)
		{
			...
			int triangle_idx;
			// FIXED: Added t_temp > 0.0001 check
			if (ray_mesh_intersect(ray, *mesh, &t_temp, &triangle_idx) && t_temp > 0.0001 && t_temp < max_dist)
				return (1);
		}
		else if (current->type == CONE)
		{
			...
			// FIXED: Added t_temp > 0.0001 check
			if (ray_cone_intersect(ray, *cone, &t_temp) && t_temp > 0.0001 && t_temp < max_dist)
				return (1);
		}
		else if (current->type == CUBE)
		{
			...
			// FIXED: Added t_temp > 0.0001 check
			if (ray_cube_intersect(ray, *cube, &t_temp) && t_temp > 0.0001 && t_temp < max_dist)
				return (1);
		}
...
// // You can also improve your original function
// int	is_in_shadow(t_scene *scene, t_vec3 hit_point, t_vec3 light_dir, double light_distance)
// {
// 	t_ray		shadow_ray;
// 	double		shadow_bias = 0.002; // Slightly larger bias since we're not using surface normal
...
// 	shadow_ray.origin = vec3_add(hit_point, vec3_scale(light_dir, shadow_bias));
// 	shadow_ray.direction = light_dir;

// 	if (any_intersection(scene, shadow_ray, light_distance - shadow_bias))
// 		return (1);
// 	return (0);
// }
