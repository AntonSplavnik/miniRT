...
/*   Updated: 2025/06/03 12:42:46 by abillote         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//    // If no bump map, return original normal
//    if (!material->has_bump_map || !material->bump_map)
//        return hit_record->normal;

//    // Calculate object-specific tangent vectors for better bump mapping
//    t_vec3 tangent, bitangent;

//    switch (hit_record->object->type)
//    {
//        case SPHERE:
...
//    // Sample the bump map and calculate gradients
//    float height_center = sample_bump_map(material->bump_map, hit_record->uv.u, hit_record->uv.v);

//    float du = 1.0f / material->bump_map->width;
//    float dv = 1.0f / material->bump_map->height;
...
//    float du_height = (height_u - height_center);
//    float dv_height = (height_v - height_center);

//    // Calculate the perturbed normal in tangent space
//    t_vec3 bump_offset;
//    bump_offset.x = du_height * tangent.x + dv_height * bitangent.x;
//    bump_offset.y = du_height * tangent.y + dv_height * bitangent.y;
//    bump_offset.z = du_height * tangent.z + dv_height * bitangent.z;

//    // Add the bump offset to the original normal
//    t_vec3 perturbed_normal = vec3_add(hit_record->normal, bump_offset);
...
t_vec3 calculate_bump_normal(t_hit_record *hit_record)
{
    ...
    // **MUCH STRONGER BUMP EFFECT**
    float bump_strength = 0.5f; // 10x stronger than before
    ...
    //// **DEBUG: Only print for significant transitions**
    //if (fabs(gradient_u) > 0.01f || fabs(gradient_v) > 0.01f)
    //{
    //    printf("DEBUG: UV=(%.3f,%.3f) height=%.3f->%.3f->%.3f gradients=(%.3f,%.3f)\n",
    //           hit_record->uv.u, hit_record->uv.v, height_center, height_u, height_v,
    //           gradient_u, gradient_v);
    //}
