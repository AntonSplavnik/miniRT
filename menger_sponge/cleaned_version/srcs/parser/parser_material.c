...
/*   Updated: 2025/06/04 15:45:46 by abillote         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int get_property_color(t_scene *scene, const char *material_block, const char *property, t_color *color)
{
	...
	if (p == prop_pos)
		parse_error(scene, "Failed to parse red component of checker color");
	values_read++;
	...
	if (*p == ',')
	{
		...
		if (p == prop_pos)
			parse_error(scene, "Failed to parse green component of checker color");
		values_read++;
	}
	...
	if (*p == ',')
	{
		...
		if (p == prop_pos)
			parse_error(scene, "Failed to parse blue component of checker color");
		values_read++;
	}
	...
	{
		parse_error(scene, "Failed to parse checker color, expected 3 components (r,g,b)");
		return (0);
	}
}
